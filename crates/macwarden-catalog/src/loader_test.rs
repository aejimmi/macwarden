...
use macwarden_core::types::{Domain, SafetyLevel, ServiceCategory, ServiceInfo, ServiceState};
use macwarden_core::{
    find_group, find_groups_for_service, resolve_extends, resolve_group_services, validate_profile,
};
...
fn test_builtin_profile_names() {
    ...
    assert!(names.contains(&"airgapped"));
    assert!(names.contains(&"studio"));
    assert!(names.contains(&"paranoid"));
}

#[test]
fn test_resolve_extends_single_level_builtin() {
    let builtins = load_builtin_profiles();
    let minimal = builtins
        .iter()
        .find(|p| p.profile.name == "minimal")
        .expect("minimal profile must exist");
    let resolved = resolve_extends(minimal, &builtins).expect("should resolve");
...
#[test]
fn test_resolve_extends_two_levels_builtin() {
    let builtins = load_builtin_profiles();
    let dev = builtins
        .iter()
        .find(|p| p.profile.name == "developer")
        .expect("developer profile must exist");
    let resolved = resolve_extends(dev, &builtins).expect("should resolve");
    ...
    assert_eq!(
        ...
        Some(&macwarden_core::CategoryAction::Allow)
    );
}
